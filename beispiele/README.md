...
|---|---|---|
| `beispiel_kette.cps` | [Ausgabe](beispiel_kette.out.txt) | **Verkettete Abfragen ohne Zugangsdaten** — das Grundmuster jeder API-Anbindung, sofort lauffähig: Ortsname → Geocoding-API → Koordinaten und Zeitzone aus der Antwort direkt in die Wetter-API → Wetter samt **Ortszeit**, Wettercode in Klartext übersetzt. `cps beispiel_kette.cps Los Angeles` fragt einen anderen Ort ab (dort ist es dann z.B. 03:45 Uhr früh). Wetterwerte und Zeiten ändern sich naturgemäß je Lauf. |
